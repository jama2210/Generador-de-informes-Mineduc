...
def generar_informes(df, carpeta, barra, estado, modo):
    ...
    for (region, deprov, modalidad), datos_grupo in grupos:
        ...
        # ======================================================
        # ✅ VARIANTE A — 1 INFORME POR R / D / M
        # ======================================================
        if "Variante A" in modo:
            ...
            for _, row in datos_grupo.iterrows():
                ...
                doc.add_heading(limpiar_valor(row["Nombre"]), level=2)
            ...
            doc.save(os.path.join(base_sub, nombre_archivo))
        ...
        # ======================================================
            ...
            for nombre, datos_persona in datos_grupo.groupby("Nombre"):
                ...
                # ORDEN por "Nombre RBD, RED, Sostenedor"
                datos_persona = datos_persona.sort_values("Nombre RBD, RED, Sostenedor")

                for _, fila_persona in datos_persona.iterrows():

                    nombre_rbd = limpiar_valor(
                        fila_persona.get("Nombre RBD, RED, Sostenedor", "No informado")
                    )

                    doc.add_heading(
                        f"Registro asociado a: {nombre_rbd}",
                        level=2
                    )
